...
```bash
git clone https://github.com/LambdaTest/karate-selenium-sample
...
Open `src/test/java/examples/users/UITest.feature` and add or remove capabilities as needed:
...
Configure tunnel in your feature file capabilities:

```
def session = { capabilities: { browserName: 'chrome', tunnel: true }, desiredCapabilities: { browserName: 'chrome', tunnel: true } }
```
